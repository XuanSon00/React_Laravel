...
const getSubjects = async (page = 1 ,limit = 10, grade = '', searchTerm='') => {
  try {
    const response = await axios.get(`http://localhost:8000/api/subjects?page=${page}&limit=${limit}&grade=${grade}`,{
      params: {
        page,
        limit,
        grade,
